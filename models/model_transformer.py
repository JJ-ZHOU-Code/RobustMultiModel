...
class Transformer_G(nn.Module):
    ...
    def forward(self, features):
        ...
        muQuery = self.muQuery.expand(features.shape[0], -1, -1).cuda()
        sigmaQuery = self.sigmaQuery.expand(features.shape[0], -1, -1).cuda()
        h = torch.cat((muQuery, sigmaQuery, features), dim=1)
